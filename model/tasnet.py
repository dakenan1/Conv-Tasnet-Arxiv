...
class TasNet(nn.Module):                        #继承了nn.父类
    ...
    def __init__(self,
        ...
        self.encode = nn.Sequential(                                        
            nn.Conv1d(1, autoencoder_channels,                              # 带self和不带??  类内调用和函数内调用 
                      kernel_size=self.autoencoder_kernel_size,
                      stride=self.autoencoder_stride),
            nn.ReLU(),
        )
        ##############Separator################
        self.encode_norm = normalization('gLN', autoencoder_channels)           # LayerNorm

        self.conv1 = nn.Conv1d(autoencoder_channels, bottleneck_channels,          # 1*1 conv
                               kernel_size=1)

        self.separation = nn.ModuleList()  # 创建与Module相关联的List,用来存储任意数量的nn. module
        for i in range(num_repeat):
            for j in range(num_blocks):
                dilation = int(2 ** j)
                conv = Conv1dBlock(bottleneck_channels, convolution_channels,       # TCN block中的一层
                                   convolution_kernel_size, dilation, self.stride,
                                   self.normalization_type, self.causal)
                self.separation.append(conv)

        self.conv2 = nn.Conv1d(bottleneck_channels,                            # 1*1Conv
        ...
        }[self.active_func]                          #relu比sigmoid效果更好
    ...
    def loss(self, output, source, device):
        # osnr
        loss = permute_si_sdr(output, source, device, True)
        return loss
