...
def si_sdr(estimated, original):
    ...
    # # optical sdr
    # target = pow_p_norm(estimated) * original / (pow_norm(estimated, original) + EPS)
    
    noise = estimated - target
    sdr = 10 * torch.log10(pow_p_norm(target) / (pow_p_norm(noise) + EPS) + EPS)
    return sdr.squeeze_(dim=-1)

def o_sdr(estimated, original):
    # estimated = remove_dc(estimated)
    # original = remove_dc(original)
...
def permute_si_sdr(est, src, device, if_osdr):
    ...
    if if_osdr:
        pair_wise_sdr = o_sdr(est, src) # [batch_size, nspk, nspk]
    else:
        pair_wise_sdr = si_sdr(est, src)
