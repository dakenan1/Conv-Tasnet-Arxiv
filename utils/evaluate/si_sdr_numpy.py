...
def o_sdr(estimated, original):
    # # optical sdr
    target = pow_np_norm(estimated) * original / pow_norm(estimated, original)
    noise = estimated - target
    return 10 * np.log10(pow_np_norm(target) / pow_np_norm(noise))


def permute_si_sdr(e1, e2, c1, c2, o_sisdr):
    if not o_sisdr:
        sdr1 = si_sdr(e1, c1) + si_sdr(e2, c2)
        sdr2 = si_sdr(e1, c2) + si_sdr(e2, c1)
    else:
        sdr1 = o_sdr(e1, c1) + si_sdr(e2, c2)
        sdr2 = o_sdr(e1, c2) + si_sdr(e2, c1)        
    if sdr1 > sdr2:
        return sdr1 * 0.5
    else:
        return sdr2 * 0.5
