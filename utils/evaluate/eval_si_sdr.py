...
def eval_si_sdr(ori_dir, mdl_dir):
    ...
    for i in range(len(wavs)):
        ...
        si_sdr = permute_si_sdr(est_wav1, est_wav2, ori_wav1, ori_wav2, False)
        o_sdr = permute_si_sdr(est_wav1, est_wav2, ori_wav1, ori_wav2, True)
    ...
    mean_o_sdr = np.mean(np.array(O_SDR)) if O_SDR else 0.0
    ...
    print('The mean O-SDR is {:.4f}'.format(mean_o_sdr))
    sys.stdout.flush()
